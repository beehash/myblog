...
export default function Pagenation({pageSize, current, setCurrent, total, change}: any) {
  const size = Math.ceil(total / pageSize);
  function generateTemplate() {
    const listTemplate = [];
    for(let i = 0; i < size; i++){
      listTemplate.push(<span className={styles['page-btn']} key={i} onClick={() => (setCurrent(i + 1), change('page', i+1))}>{i+1}</span>);
    }
    return listTemplate;
  }
  ...
  return (
    <div className={styles.pagenation + ' mt-16 '}>
      <span className={styles['page-btn'] + ' mx-4 ' + (current === 1 ? 'disabled' : '')}
        onClick={() => current !== 1 && setCurrent(current - 1) && change('prev', current + 1)}>
      ...
      <span className={styles['page-btn'] + ' mx-4 ' + (current === size ? 'disabled' : '')}
        onClick={() => current !== size && setCurrent(current + 1) && change('next', current - 1)}>
